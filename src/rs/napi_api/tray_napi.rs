...
fn create_tray(label: String, data: String) -> Result<()> {
  ...
  crate::with_app(|app| {
    app.create_tray(label, &data).map_err(Error::from_reason)
  })
}
...
fn remove_tray(label: String) -> Result<()> {
  crate::with_app(|app| {
    app.remove_tray(&label).map_err(Error::from_reason)
  })
}
...
fn set_tray_icon(label: String, data: String) -> Result<()> {
  ...
  crate::with_app(|app| {
    app.set_tray_icon(&label, icon_path).map_err(Error::from_reason)
  })
}
...
fn set_tray_menu(label: String, data: String) -> Result<()> {
  ...
  crate::with_app(|app| {
    app.set_tray_menu(&label, menu_label).map_err(Error::from_reason)
  })
}
...
fn set_tray_tooltip(label: String, data: String) -> Result<()> {
  ...
  crate::with_app(|app| {
    let tray = app.trays.get(&label).ok_or_else(|| Error::from_reason(format!("tray '{}' does not exist", label)))?;
