...
where F: FnOnce(&crate::window::BrowserWindow) -> Result<R> {
  crate::with_app(|app| {
    let win = app.get_window(label).ok_or_else(|| Error::from_reason(format!("window '{}' does not exist", label)))?;
    f(win)
  })
}
...
fn window_close(label: String) -> Result<()> {
  crate::with_app(|app| { app.close_window(&label); Ok(()) })
}

#[napi]
fn window_request_redraw(label: String) -> Result<()> {
  with_window(&label, |w| { w.request_redraw(); Ok(()) })
}

#[napi]
fn window_set_url(label: String, url: String) -> Result<()> {
  with_window(&label, |w| w.set_url(&url).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_load_url_with_headers(label: String, data: String) -> Result<()> {
  let data = parse_json(&data);
  let url = data.get("url").and_then(Value::as_str).ok_or_else(|| Error::from_reason("url is required"))?.to_string();
  let headers = headers_from_value(data.get("headers").unwrap_or(&Value::Null)).map_err(Error::from_reason)?;
  with_window(&label, |w| w.load_url_with_headers(&url, headers).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_url(label: String) -> Result<String> {
  with_window(&label, |w| w.url().map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_evaluate_script(label: String, script: String) -> Result<()> {
  with_window(&label, |w| w.evaluate_script(&script).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_print(label: String) -> Result<()> {
  with_window(&label, |w| w.print().map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_open_devtools(label: String) -> Result<()> {
  with_window(&label, |w| { w.open_devtools(); Ok(()) })
}

#[napi]
fn window_close_devtools(label: String) -> Result<()> {
  with_window(&label, |w| { w.close_devtools(); Ok(()) })
}
...
fn window_zoom(label: String, scale: f64) -> Result<()> {
  with_window(&label, |w| w.zoom(scale).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_clear_all_browsing_data(label: String) -> Result<()> {
  with_window(&label, |w| w.clear_all_browsing_data().map_err(|e| Error::from_reason(e.to_string())))
}
...
fn window_set_background_color(label: String, data: String) -> Result<()> {
  ...
  with_window(&label, |w| w.set_background_color(color).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_set_window_background_color(label: String, data: String) -> Result<()> {
  let v = parse_json(&data);
  let color = if v.is_null() { None } else { Some(color_value(&v).map_err(Error::from_reason)?) };
  with_window(&label, |w| { w.set_window_background_color(color); Ok(()) })
}
...
fn window_inner_position(label: String) -> Result<String> {
  with_window(&label, |w| {
    let p = w.inner_position().map_err(|e| Error::from_reason(e.to_string()))?;
...
fn window_outer_position(label: String) -> Result<String> {
  with_window(&label, |w| {
    let p = w.outer_position().map_err(|e| Error::from_reason(e.to_string()))?;
    Ok(pos_to_json(p.x as f64, p.y as f64))
  })
}
...
fn window_set_outer_position(label: String, data: String) -> Result<()> {
  let pos = position_from_value(&parse_json(&data)).ok_or_else(|| Error::from_reason("position is invalid"))?;
  with_window(&label, |w| { w.set_outer_position(pos); Ok(()) })
}
...
fn window_set_min_inner_size(label: String, data: String) -> Result<()> {
  let v = parse_json(&data);
  with_window(&label, |w| {
    if v.is_null() { w.set_min_inner_size::<tao::dpi::Size>(None); }
    else { w.set_min_inner_size(Some(size_value(&v).map_err(Error::from_reason)?)); }
    Ok(())
  })
}
...
fn window_set_max_inner_size(label: String, data: String) -> Result<()> {
  let v = parse_json(&data);
  with_window(&label, |w| {
    if v.is_null() { w.set_max_inner_size::<tao::dpi::Size>(None); }
    else { w.set_max_inner_size(Some(size_value(&v).map_err(Error::from_reason)?)); }
    Ok(())
  })
}
...
fn window_set_inner_size_constraints(label: String, data: String) -> Result<()> {
  ...
  with_window(&label, |w| { w.set_inner_size_constraints(c); Ok(()) })
}
...
fn window_request_user_attention(label: String, data: String) -> Result<()> {
  ...
  with_window(&label, |w| { w.request_user_attention(attention); Ok(()) })
}
...
fn window_set_title(label: String, title: String) -> Result<()> {
  with_window(&label, |w| { w.set_title(&title); Ok(()) })
}
...
fn window_set_visible(label: String, visible: bool) -> Result<()> {
  with_window(&label, |w| { w.set_visible(visible); Ok(()) })
}
...
fn window_focus(label: String) -> Result<()> {
  with_window(&label, |w| { w.focus_window(); Ok(()) })
}
...
fn window_set_resizable(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_resizable(value); Ok(()) })
}
...
fn window_set_minimizable(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_minimizable(value); Ok(()) })
}
...
fn window_set_maximizable(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_maximizable(value); Ok(()) })
}
...
fn window_set_closable(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_closable(value); Ok(()) })
}
...
fn window_set_enabled_buttons(label: String, data: String) -> Result<()> {
  ...
  with_window(&label, |w| { w.set_enabled_buttons(c, m, x); Ok(()) })
}

#[napi]
fn window_enabled_buttons(label: String) -> Result<String> {
  with_window(&label, |w| {
    let buttons: Vec<String> = w.enabled_buttons().into_iter().map(|b| b.to_string()).collect();
...
fn window_set_minimized(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_minimized(value); Ok(()) })
}
...
fn window_set_maximized(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_maximized(value); Ok(()) })
}
...
fn window_fullscreen(label: String, data: String) -> Result<()> {
  ...
  crate::with_app_el(|app, el| {
    ...
    with_window(&label, |w| { w.set_fullscreen(Some(fs)); Ok(()) })
  })
}

#[napi]
fn window_unfullscreen(label: String) -> Result<()> {
  with_window(&label, |w| { w.set_fullscreen(None); Ok(()) })
}
  ...
  crate::with_app_el(|app, el| {
    with_window(&label, |w| {
      Ok(match w.fullscreen() {
        Some(tao::window::Fullscreen::Borderless(Some(monitor))) => {
          app.monitor_id(el, &monitor).map(|id| json!(id).to_string()).unwrap_or_else(|| "true".to_string())
        }
...
fn window_set_decorations(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_decorations(value); Ok(()) })
}
...
fn window_set_always_on_top(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_always_on_top(value); Ok(()) })
}
...
fn window_set_theme(label: String, data: String) -> Result<()> {
  ...
  with_window(&label, |w| { w.set_theme(theme); Ok(()) })
}
...
fn window_set_content_protection(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_content_protection(value); Ok(()) })
}

#[napi]
fn window_set_visible_on_all_workspaces(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_visible_on_all_workspaces(value); Ok(()) })
}
...
fn window_set_cursor_icon(label: String, cursor: String) -> Result<()> {
  ...
  with_window(&label, |w| { w.set_cursor_icon(icon); Ok(()) })
}

#[napi]
fn window_set_cursor_position(label: String, data: String) -> Result<()> {
  let pos = position_from_value(&parse_json(&data)).ok_or_else(|| Error::from_reason("position is invalid"))?;
  with_window(&label, |w| w.set_cursor_position(pos).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_set_cursor_grab(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| w.set_cursor_grab(value).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_set_cursor_visible(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| { w.set_cursor_visible(value); Ok(()) })
}

#[napi]
fn window_drag_window(label: String) -> Result<()> {
  with_window(&label, |w| w.drag_window().map_err(|e| Error::from_reason(e.to_string())))
}
...
fn window_drag_resize_window(label: String, direction: String) -> Result<()> {
  ...
  with_window(&label, |w| w.drag_resize_window(dir).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_set_ignore_cursor_events(label: String, value: bool) -> Result<()> {
  with_window(&label, |w| w.set_ignore_cursor_events(value).map_err(|e| Error::from_reason(e.to_string())))
}

#[napi]
fn window_cursor_position(label: String) -> Result<String> {
  with_window(&label, |w| {
    let p = w.cursor_position().map_err(|e| Error::from_reason(e.to_string()))?;
...
fn evaluate_script(label: String, script: String, callback: JsFunction) -> Result<()> {
  let tsfn: napi::threadsafe_function::ThreadsafeFunction<String, napi::threadsafe_function::ErrorStrategy::Fatal> = callback
  ...
  with_window(&label, |w| {
    w.evaluate_script_with_callback(&script, move |result| {
      tsfn.call(result, napi::threadsafe_function::ThreadsafeFunctionCallMode::NonBlocking);
...
fn rpc_invoke(label: String, method: String, data: String, callback: JsFunction) -> Result<()> {
  ...
  crate::with_app(|app| {
    let win = app.get_window(&label).ok_or_else(|| Error::from_reason(format!("window '{}' does not exist", label)))?;
    let rpc_id = win.rpc_state.lock().map_err(|e| Error::from_reason(e.to_string()))?
      .assign_host_request_id(move |result| {
        let json = match result {
          Ok(data) => serde_json::to_string(&json!({"data": data})).unwrap_or_default(),
          Err(error) => serde_json::to_string(&json!({"error": error})).unwrap_or_default(),
        };
    ...
    win.evaluate_script(&js).map_err(|e| Error::from_reason(e.to_string()))
  })
}
...
/// 回复 views:// 自定义协议请求
#[napi]
fn protocol_response(label: String, request_id: String, status_code: u32, headers: String, body: String) -> Result<()> {
  let headers_value: Value = serde_json::from_str(&headers).unwrap_or(Value::Object(serde_json::Map::new()));
  crate::with_app(|app| {
    let win = app.get_window(&label).ok_or_else(|| Error::from_reason(format!("window '{}' does not exist", label)))?;
    let body = if body.is_empty() {
      ...
      BASE64.decode(&body).map_err(|e| Error::from_reason(format!("invalid base64 data: {}", e)))?
    };
    ...
    let response = response_builder.body(body).map_err(|e| Error::from_reason(format!("failed to build response: {}", e)))?;
    let responder = win.protocol_state.lock().map_err(|e| Error::from_reason(e.to_string()))?.remove(&request_id);
    match responder {
      Some(responder) => { responder.respond(response); Ok(()) }
      None => Err(Error::from_reason(format!("protocol request '{}' not found", request_id))),
    }
  })
}
