...
fn monitor_info(
    el: &tao::event_loop::EventLoopWindowTarget<crate::application::Action>,
...
fn monitor_from_point(data: String) -> Result<String> {
    ...
    let x = v
        ...
        .ok_or_else(|| Error::from_reason("x is required"))?;
    let y = v
        .get("y")
        .and_then(Value::as_f64)
        .ok_or_else(|| Error::from_reason("y is required"))?;
    crate::with_app_el(|_app, el| {
        Ok(el
            .available_monitors()
            .find(|monitor| {
