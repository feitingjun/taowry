...
impl Application {
  ...
  pub fn set_window_menu(&self, window_label: &str, menu_label: &str) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      menu
        .init_for_hwnd(window.window.hwnd())
  ...
  fn build_menu_item(
    ...
    let item = match item_type {
      ...
      "predefined" => {
        let kind = data.get("item").and_then(Value::as_str).unwrap_or("separator");
        ManagedMenuItem::Predefined(build_predefined_item(kind, data.get("text").and_then(Value::as_str))?)
      }
      "check" => {
        let checked = data.get("checked").and_then(Value::as_bool).unwrap_or(false);
      ...
      _ => {
        let item = MenuItem::with_id(item_id.clone(), text, enabled, parse_accelerator(data)?);
        self
          .menu_items
...
impl ManagedMenu {
  ...
  pub fn as_root_menu(&self) -> Result<Menu, String> {
    match self {
      ...
      ManagedMenu::Submenu(_) => Err("a submenu cannot be used as an application/window menu".to_string()),
    }
  }
}
...
fn build_predefined_item(
  kind: &str,
  text: Option<&str>,
) -> Result<PredefinedMenuItem, String> {
  Ok(match kind {
...
fn apply_webview_options<'a>(
  ...
  if let Some(url) = data.get("url").and_then(Value::as_str) {
    ...
    // Read the file and load as HTML instead, with a <base> tag for relative paths.
    if url.starts_with("file:") && !url.starts_with("file://") {
      // Normalize "file:" to "file://" for path extraction
      ...
      if let Ok(html_content) = std::fs::read_to_string(file_path) {
        let dir = std::path::Path::new(file_path)
          ...
          .unwrap_or_default();
        let html_with_base = format!(
          "<base href=\"{}\">{}",
          dir, html_content
        );
        builder = builder.with_html(html_with_base);
      } else {
    ...
    } else if let Some(headers) = data.get("headers") {
  ...
  let is_transparent = data.get("transparent").and_then(Value::as_bool).unwrap_or(false);
  ...
  #[cfg(target_os = "macos")]
  {
    ...
    if let Some(position) = data.get("trafficLightPosition") {
      if let Some(pos) = position_from_value(position) {
        builder = builder.with_traffic_light_inset(tao::dpi::LogicalPosition::new(pos.x, pos.y));
      }
    }
  }
  Ok(builder)
...
pub fn fullscreen_from_value(
  ...
  if let Some(monitor_id) = value.as_u64() {
    return Some(Fullscreen::Borderless(app.monitor_by_id(event_loop, monitor_id)));
  }
