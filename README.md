# [node-webview](https://github.com/feitingjun/node-webview)

在 Node.js 中使用 [tao](https://github.com/nickel-org/tao) 和 [wry](https://github.com/nickel-org/wry) 创建跨平台 WebView 窗口。

基于 **tao**（跨平台窗口管理）+ **wry**（WebView 渲染）+ **Node.js** 子进程 IPC 通信。

https://github.com/feitingjun/node-webview
...
```typescript
import { Application, BrowserWindow } from 'node-webview'
...
```typescript
import { Application } from 'node-webview'
...
```typescript
import { BrowserWindow } from 'node-webview'
...
```typescript
import { RPCInterface, RPCSchema, defineRPC, BrowserWindow } from 'node-webview'
...
WebView 端通过 `node-webview/client` 子路径导入：

```typescript
import { defineRPC, RPCInterface, RPCSchema } from 'node-webview/client'
...
```typescript
import { getWindow } from 'node-webview'
...
```typescript
import { defineRPC } from 'node-webview'       // Host 端
import { defineRPC } from 'node-webview/client' // WebView 端
```
