...
import { Application, BrowserWindow, defineRPC, RPCSchema, Tray } from '../src/ts/index'
...
const rpc = defineRPC<RPC>({
  ...
  messages: {
    update: async data => {
      console.log('[host] received message:', data.message)
      // 发送消息回 WebView，触发 _handleSend
      win.rpc?.messages.update({ message: 'host-reply: ' + data.message })
    }
  }
})
